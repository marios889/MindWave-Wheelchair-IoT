...
class PirsensorCard extends StatefulWidget {
  final SmartDevice device;

  const PirsensorCard({super.key, required this.device});

  @override
  State<PirsensorCard> createState() => _FullWidthSensorCardState();
}

class _FullWidthSensorCardState extends State<PirsensorCard> {
  @override
  void initState() {
    super.initState();
    if (!NetworkWatchdogManager.isPirStreaming()) {
      _sendDedicatedPIRPing();
    } else {
      print(
        "🛡️ Watchdog: PIR packets already incoming on page entry. Suppressed ping.",
      );
    }
  ...
  Future<void> _sendDedicatedPIRPing() async {
    try {
              ...
              children: [
                Container(
                  ...
                  child: SizedBox(
                    ...
                    height: 50.0,
                    child: Image.asset(
                      "assets/images/motion-sensor.png",
                      fit: BoxFit.cover,
                    ),
                  ),
                ),
